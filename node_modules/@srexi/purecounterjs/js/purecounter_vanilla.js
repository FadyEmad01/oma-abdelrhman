...
function startCounter(element, config) {
    ...
    var counterWorker = setInterval(() => {
        ...
        // Stop the interval if the next number exceeds the end value
        if ((countMode == "inc" && nextNum > config.end) || (countMode == "dec" && nextNum < config.end)) {
            clearInterval(counterWorker);
        }
    }, config.delay);
}
...
function formatNumber(number, config) {
    ...
    // Now format the number to string base on its locale
    number = config.formater
        ? number.toLocaleString(locale, strConfig)
        : parseFloat(number).toFixed(config.decimals); // Use toFixed to preserve decimals
